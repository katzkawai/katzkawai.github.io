...
- `CACHE_NAME = 'kklab-cache-v8'`。**ナビゲーションリクエスト（HTML）は network-first** — 毎回ネットワークから取得してキャッシュを更新し、オフライン時はキャッシュ → `/offline.html` の順にフォールバック。そのためページ内容の変更（作品カード追加など）にキャッシュバンプは不要。その他のリソース（CSS/JS/画像/パーシャル）は cache-first + ランタイムキャッシュ（status 200 かつ type `basic` のみ保存）。
