...
Cache name `kklab-cache-v8`. **Navigation requests (HTML) are network-first** — fresh HTML is fetched and re-cached on every visit, falling back to cache and then `/offline.html` when offline — so page-content edits (e.g. adding a work card) do NOT require a cache bump. All other resources (CSS/JS/images/partials) are cache-first. **`urlsToCache` is a hardcoded precache list** (includes the HTML pages, `style.css`, `js/*.js`, and the `includes/*.html` partials). When adding a core page/asset that must work offline, or when changing a cache-first asset that must reach returning visitors, add/keep its path there AND bump `CACHE_NAME` (e.g. `-v9`) so the old cache is purged on `activate`. `offline.html` is intentionally self-contained (inline styles) so it renders without `style.css`.
